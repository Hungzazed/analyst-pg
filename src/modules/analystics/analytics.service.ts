...
export class AnalyticsService {
    ...
    return {
      ...
      pages: rows.map((row) => {
        ...
        return {
          ...
          bounceRate:
            entryCount > 0
              ? Math.round((bounceCount / entryCount) * 1000) / 1000
              : 0,
          exitRate:
            row.count > 0
              ? Math.round((exitCount / row.count) * 1000) / 1000
              : 0,
        };
      }),
    };
  ...
  async getTrafficSources(
    ...
    for (const event of events) {
      const source = this.resolveTrafficSource(
        event.referrer,
        event.metadata,
        website.domain,
      );
  ...
  async getBehavior(
    ...
    return {
      ...
      avgPagesPerSession:
        sessions.length > 0
          ? Math.round((totalPages / sessions.length) * 1000) / 1000
          : 0,
      transitions: {
  ...
  async getRealtime(
    ...
    return {
      ...
      activeSessionsLimit,
      activeSessions,
      currentPages: this.toSortedRows(currentPageCounts).slice(
        0,
        activeSessionsLimit,
      ),
  ...
  async getFunnel(userId: string, websiteId: string, query: FunnelQueryDto) {
    ...
    return {
      ...
      totalConversionRate:
        landingCount > 0
          ? Math.round((conversionCount / landingCount) * 1000) / 1000
          : 0,
  ...
  private buildOverviewSnapshot(snapshot: RangeSnapshot) {
    ...
    return {
      ...
      averageSessionDurationMs: this.calculateAverageSessionDuration(
        snapshot.sessions,
      ),
  ...
  private buildDailyBounceRates(
    ...
    for (const [key, totalSessions] of daySessions.entries()) {
      ...
      result.set(
        key,
        totalSessions > 0
          ? Math.round((bounced / totalSessions) * 1000) / 1000
          : 0,
      );
    }
  ...
  private resolveTrafficSource(
    ...
    const utmMedium = this.readMetadataString(metadata, [
      'utm_medium',
      'medium',
    ]);
    const utmCampaign = this.readMetadataString(metadata, [
      'utm_campaign',
      'campaign',
    ]);
  ...
  private calculatePeriodIndex(
    ...
    const unitMs =
      granularity === 'week' ? 7 * 24 * 60 * 60 * 1000 : 24 * 60 * 60 * 1000;
    return Math.floor(
      (this.bucketDate(date, granularity).getTime() - cohortStart.getTime()) /
        unitMs,
    );
  }
