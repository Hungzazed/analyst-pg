...
describe('Auth API', () => {
  ...
  beforeEach(() => {
    ...
    jwtStrategyMock.getRefreshTokenExpiryDate.mockReturnValue(
  ...
  it('registers a user and never returns password', async () => {
    ...
    expect(await bcrypt.compare(rawPassword, createCall.data.password)).toBe(
      true,
    );
  });
  ...
  it('logs in successfully, stores hashed refresh token, and sets refresh cookie', async () => {
    ...
    expect(
      setCookies.some((cookie) =>
        cookie.startsWith(`refreshToken=${refreshToken};`),
      ),
    ).toBe(true);
    expect(setCookies.some((cookie) => cookie.includes('HttpOnly'))).toBe(true);
    expect(setCookies.some((cookie) => cookie.includes('Path=/auth'))).toBe(
      true,
    );
    ...
    expect(await bcrypt.compare(refreshToken, createCall.data.token)).toBe(
      true,
    );
  ...
  it('refreshes access token when refresh token is valid', async () => {
    ...
    expect(
      setCookies.some((cookie) =>
        cookie.startsWith('refreshToken=refresh-token-rotated;'),
      ),
    ).toBe(true);
    ...
    const createCall = prismaMock.refreshToken.create.mock.calls[0][0];
    expect(
      await bcrypt.compare('refresh-token-rotated', createCall.data.token),
    ).toBe(true);
